...
const CourseKnowledgeCheck: React.FC = (props: any) => {
  ...
  const [showSeventhActionResults, setShowSeventhActionResults] =
    useState(false);
        ...
        if (userAnswerData && typeof userAnswerData === 'object') {
          ...
          if (
            ...
            setSubmitButtonCondtion(
              currentCardIndex === examPlayData?.length - 1,
            );
  ...
  const handleDraggleCheck = () => {
    ...
    const callback = (res: any) => {
      ...
      if (res !== 'error') {
        ...
        const correctAnswer =
          res?.questionDetails?.questionsList[0]?.examQuesOptionsList;
  ...
  const mcqQuetionCheck = () => {
    ...
    const callback = (res: any) => {
      ...
      if (res !== 'error') {
        ...
        const correctAnswer =
          res?.questionDetails?.questionsList[0]?.examQuesOptionsList?.findIndex(
            (item: any) => item?.isCorrectAnswer === true,
          );
          ...
          {hasFilterData?.map((item: any, index: number) => (
            <Pressable
              ...
              style={{
                ...
                alignItems: 'center',
                backgroundColor:
                  selectedIndex !== index ? COLORS.WHITE : '#DDE1F6',
                margin: 5,
  ...
  const coursePlayFunction = () => {
    ...
    const currentChapterIndex = coursePlayData?.chapters.findIndex(
      (chapter: any) =>
        chapter?.kcExam?.id === props?.route?.params?.kcData?.id,
    );
    if (
      currentChapterIndex !== -1 &&
      currentChapterIndex + 1 < coursePlayData?.chapters.length
    ) {
      const secondChapterItem =
        coursePlayData?.chapters[currentChapterIndex + 1];
      resetQuestionState();
    ...
    if (
      ...
      setFifthDraggableData((prevData: any) => {
        const isSame =
          ...
          prevData.every(
            (item: any, index: number) => item === hasDragerData[index],
          );
        ...
        <>
              ...
              {Array.isArray(examPlayCard?.questionsList) &&
                examPlayCard?.questionsList?.map((item: any, index: any) => (
                  <>
                    ...
                    {seeCorrectAns && (
                      <View style={styles.incorrectAnswerContainer}>
                        ...
                        <Pressable onPress={() => handleSeeCorrectAnswer(item)}>
                          <RNText
                            underline
                            style={styles.seeCorrectAnswerText}
                    ...
                    {seeYourAns && (
                      <Pressable
                        onPress={() => handleSeeYourAnswer(item)}
                        style={styles.seeYourAnswerContainer}>
                        <RNText
                          underline
                          style={styles.seeYourAnswerText}
                          textColor={COLORS.PRIMARY}
                          small>
                          See Your Answer
                        </RNText>
                  ...
                  <>
                    ...
                    <View style={{marginTop: scale(30)}}>
                      {!optionalCheckWrongRight
                        ...
                        : multiSelectedResponse?.questionDetails?.questionsList[0]?.examQuesOptionsList?.map(
                            (optionItem: any, index: number) => {
                              const isCorrect = optionItem.isCorrectAnswer;
                              let isUserAnswer;
                              const userSelectedIds =
                                multiSelectedResponse?.userAnswerData?.optionIds?.split(
                                  '|',
                                ) || [];
                              isUserAnswer = userSelectedIds.includes(
                                optionItem.id,
                              );
                              ...
                              const anyIncorrectSelected =
                                multiSelectedResponse?.questionDetails?.questionsList[0]?.examQuesOptionsList.some(
                                  (option: any) =>
                                    !option.isCorrectAnswer &&
                                    userSelectedIds.includes(option.id),
                                );
                              ...
                              <Pressable
                                ...
                                style={[
                                  styles.optionContainer,
                                  {
                                    height: 50,
                                    backgroundColor:
                                      booleanCorrectAns?.correctAnswer ===
                                        optionItem?.text &&
                                      checkTrueFalseIndex === index
                                        ? COLORS.LIGHTGREEN // Correct answer selected
                                        : booleanCorrectAns?.correctAnswer !==
                                            optionItem?.text &&
                                          checkTrueFalseIndex === index
                      ...
                      <View style={{marginTop: scale(30)}}>
                        {showFifthDraggleResults && (
                          <RNText
                            ...
                            style={[
                              styles.seeCorrectAnswerText,
                              {marginBottom: scale(10), marginTop: scale(-20)},
                            ]}
                            textColor={COLORS.PRIMARY}
                            small>
                            {fifthSeeCorrectAnsState
                        ...
                        {!showFifthDraggleResults ? (
                          ...
                          fifthDraggableData?.map(
                            (item: any, index: number) => {
                              const correctPosition =
                                draggableRightWrongAns.findIndex(
                    ...
                    <React.Fragment key={index}>
                      ...
                      <View
                        ...
                        {showSixthActionResults ? (
                          <>
                            <RNText textColor={COLORS.GREEN} medium>
                              Correct Answer
                            </RNText>
                            <RNText
                              ...
                              medium>
                              {parts[0]}
                              <RNText textColor={COLORS.TEXTCOLOR} bold>
                                {correctAnswer}
                              </RNText>
                              {parts[1]}
                            </RNText>
          ...
          {QuestionCheck?.MATCHLIST === examPlayType && (
            <>
              ...
              {Array.isArray(examPlayCard?.questionsList) &&
                examPlayCard?.questionsList.length > 0 && (
                      ...
                      <RNText
                        onPress={
                          showSeventhActionSheet
                            ? seventhSeeYourAns
                            : seventhSeeCorrectAns
                        }
                        style={[
                          styles.seeCorrectAnswerText,
                          {marginBottom: scale(10), marginTop: scale(0)},
                        ]}
                        textColor={COLORS.PRIMARY}
                        small>
                        {showSeventhActionSheet
              ...
              {!showSeventhActionResults
                        ...
                        <View
                          ...
                          <RNText textColor={COLORS.TEXTCOLOR} medium>
                            {(() => {
                              ...
                              const matchingItem = Array.isArray(
                                ...
                                : null;
                              return (
                                matchingItem?.examQuesOptionsList?.[0]
                                  ?.answer || ''
                              );
                            })()}
                          </RNText>
                ...
                : showSeventhActionSheet
                ? Array.isArray(matchListValue?.questionsList) &&
                  matchListValue?.questionsList.map((item: any, index: any) => (
                    <View key={index}>
                      <Pressable
                        style={[
                          styles.optionButton,
                ...
                : Array.isArray(passingMatchListData) &&
                  passingMatchListData.map((item: any, index: any) => {
                    const correctPosition =
                      matchListValue?.questionsList?.findIndex(
                        (ans: any) =>
                          ans?.examQuesOptionsList[0]?.id ===
                          item?.examQuesOptionsList[0]?.id,
                      );
                    const isCorrect = index === correctPosition;
                    return (
                      <View key={index}>
    ...
    return (
      <RNModal transparent visible={modalVisible}>
        <View
          ...
          <RNText style={{marginTop: 15, paddingBottom: 10}} large>
      ...
      <RNContainer
        ...
        <KeyboardAvoidingView
          ...
          {examPlayLoading ? (
            ...
            <RNText
              TextAlignCenter
              style={[styles.questionText, {paddingTop: 20}]}
              small
          ...
          {(() => {
            try {
              return QuestionCheck?.TEXT &&
                examPlayData?.[currentCardIndex]?.questionsList?.[0]?.type ===
                  QuestionCheck.TEXT &&
