...
} from '../FlashCardStack/module/action';
import {
...
const CoursePreview: React.FC = (props: any) => {
  ...
  const totalGamificationPoint = () => {
    const totalPoints = flashcardPreviewDetailsData?.chapters?.reduce(
      (acc: any, chapter: any) => {
        const materialsLength = chapter?.materials?.length || 0;
        const chapterPoints =
          materialsLength *
          (flashcardPreviewDetailsData?.gamificationPoints || 0);
  ...
  const PreviewMainView = () => {
    const playedTime =
      flashcardPreviewDetailsData?.userProgress?.playedTime ?? 0;
    const duration = flashcardPreviewDetailsData?.userProgress?.duration ?? 0;

    let courseProgressData =
      flashcardPreviewDetailsData?.userProgress?.status === 'COMPLETED'
        ? 1
        : playedTime / duration;
    courseProgressData =
      flashcardPreviewDetailsData?.userProgress?.status === 'IN_PROGRESS' &&
      courseProgressData === 1
        ? 0.99
        : courseProgressData;
    ...
    const formattedDuration = formatDuration(
      flashcardPreviewDetailsData?.userProgress?.duration,
    );
    const secondFormattedDuration = secondFormatDuration(
      flashcardPreviewDetailsData?.userProgress?.duration,
    );
      ...
      <>
        <View style={styles.imageContainer}>
          {flashcardPreviewDetailsData?.bannerImageUrl ? (
            <RNImage
              resizeMode="Cover"
              source={{uri: flashcardPreviewDetailsData?.bannerImageUrl}}
              style={styles.image}
            />
          ) : (
            <RNText
        ...
        <View style={styles.cardMainView}>
          <View
            ...
            <RNText
              ...
              style={[styles.cardTitle, {maxWidth: '70%'}]}
              bold
              large>
              {flashcardPreviewDetailsData?.title || ''}
            </RNText>
            {flashcardPreviewDetailsData?.userProgress?.dueDate.split('T')[0] <
              today &&
            flashcardPreviewDetailsData?.userProgress?.status?.toLowerCase() !==
              'completed' ? (
              <View
                style={{
                ...
                'completed' &&
              flashcardPreviewDetailsData?.userProgress?.isPassed != null &&
              flashcardPreviewDetailsData?.userProgress?.isPassed !==
                undefined ? (
              <View
                style={{
                  backgroundColor: flashcardPreviewDetailsData?.userProgress
                    ?.isPassed
                    ? COLORS.GREEN
                    : COLORS.RED,
                  padding: 3,
          ...
          {flashcardPreviewDetailsData?.userProgress?.status == 'CANCELLED' ? (
            <Pressable
              onPress={() => {
                if (
                  flashcardPreviewDetailsData?.enrollmentStatus == 'PENDING'
                ) {
                  console.log('pending!!');
                } else if (
                  flashcardPreviewDetailsData?.enrollmentStatus == 'APPROVED'
                ) {
              ...
              style={{
                borderWidth: 1,
                borderColor:
                  flashcardPreviewDetailsData?.enrollmentStatus == '' ||
                  flashcardPreviewDetailsData?.enrollmentStatus.toLowerCase() ==
                    'rejected'
                    ? COLORS.TRANSPARENT
                    : flashcardPreviewDetailsData?.enrollmentStatus?.toLowerCase() ==
                      'pending'
          ...
          <View style={styles.progressContainer}>
            <Progress.Bar
              ...
              progress={
                !isNaN(courseProgressData) && courseProgressData !== null
                  ? courseProgressData
                  : 0
              }
              width={scale(235)}
            ...
            <RNText style={styles.progressText} bold small>
              {!isNaN(courseFinalPercentage) && courseFinalPercentage !== null
          ...
          <View style={styles.infoContainer}>
            <View style={styles.infoRow}>
              <RNText textColor={COLORS.BORDER_COLOR} medium>
                Enrollment Date
              </RNText>
              {!flashcardPreviewDetailsData?.userProgress?.assignedDate ? (
                ...
                <RNText textColor={COLORS.TEXTCOLOR} semiBold medium>
                  {convertDateIntoDay(
                    flashcardPreviewDetailsData?.userProgress?.assignedDate,
                  )?.newConvertDate || ''}
                </RNText>
            ...
            <View style={styles.infoRow}>
              ...
              {!flashcardPreviewDetailsData?.courseType ? (
                ...
                <RNText textColor={COLORS.TEXTCOLOR} semiBold medium>
                  {flashcardPreviewDetailsData?.courseType == 'SCORM'
                    ? ' ' + formattedDuration
            ...
            {flashcardPreviewDetailsData?.isCertificateAvailable ==
            false ? null : (
              <View style={styles.infoRow}>
                <RNText textColor={COLORS.BORDER_COLOR} medium>
        ...
        <View style={[styles.cardMainView, {marginTop: scale(20)}]}>
          <View style={{flexDirection: 'row', alignItems: 'center'}}>
            <RNImage
              ...
              style={{width: scale(15), height: scale(15), left: scale(15)}}
            />
            <RNText
              textColor={COLORS.TEXTCOLOR}
              style={[styles.cardTitle, {maxWidth: '70%', left: scale(0)}]}
              bold
              large>
              {'Grade'}
            </RNText>
          ...
          <View style={styles.infoContainer}>
            <View style={{bottom: scale(10)}}>
              ...
              {scromChapterByData?.length != 0 &&
              scromChapterByData?.chapters?.length != 0 ? null : (
                <View style={styles.infoRow}>
                  <RNText textColor={COLORS.BORDER_COLOR} medium>
                    Total Gamification Points
                  </RNText>
                  {!flashcardPreviewDetailsData?.gamificationPoints &&
                  flashcardPreviewDetailsData?.gamificationPoints !== 0 ? (
                    <ShimmerPlaceHolder
                      duration={2000}
                      style={{width: 100, height: 15}}
        ...
        <>
          ...
          <View style={{width: '85%'}}>
            <RNText
              ...
              textColor={'#575757'}
              medium>
              Good job completing the assignment. Your work shows understanding
      ...
      <RNModal
        ...
        <View
          ...
          <RNText style={{marginTop: 10}} large>
            Are you sure that you want to send request for this course?
          </RNText>
  ...
  const CourseDescrption = () => {
    ...
    return (
      <View
        style={[
          styles.cardMainView,
          {marginTop: scale(20), marginBottom: scale(20)},
        ]}>
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          <RNImage
            source={IMAGES.circleInfo}
            style={{width: scale(15), height: scale(15), left: scale(15)}}
          />
          <RNText
            textColor={COLORS.TEXTCOLOR}
        ...
        <View style={{left: scale(20), width: '85%', bottom: 10}}>
          <RenderHTML
            contentWidth={width}
            source={{html: truncate(truncatedText, 150)}}
          />
          <Pressable
            ...
            <RNText
              style={{marginBottom: scale(10), bottom: 5, marginTop: scale(5)}}
              textColor={COLORS.BORDER_COLOR}
              medium>
              {flashcardPreviewDetailsData?.longDescription?.length > 150
                ? 'Read More'
                : ''}
            </RNText>
          </Pressable>
  ...
  const CourseDiscrptionSheet = () => {
    //ActionSheetRef?.current?.hide();
    const {width} = useWindowDimensions();
    return (
      <RNActionSheet ActionSheetRef={ActionSheetRef}>
        <RNImage
          source={IMAGES.ActionSheetIcon}
          style={{alignSelf: 'center', width: scale(50)}}
        />
        <View style={styles.actionsheet}>
          <RNText style={{marginBottom: 5}} bold extraLarge>
            About Course
          </RNText>
          <ScrollView
            ...
            <RenderHTML
              contentWidth={width}
              source={{
                html: flashcardPreviewDetailsData?.longDescription || '',
              }}
            />
          </ScrollView>
    ...
    return (
      <>
        <View
          style={[
            styles.examMainView,
            {marginTop: scale(20), alignItems: 'center'},
          ]}>
          <View style={styles.examHeader}>
            ...
            <View style={styles.examQuestions}>
              <RNImage
                source={IMAGES.clipboardQuestion}
              ...
              <RNText
                ...
                textColor={COLORS.BORDER_COLOR}
                medium>
                {flashcardPreviewDetailsData?.exam?.questionsCount || 0}{' '}
                questions
              </RNText>
    ...
    return (
      ...
      <>
        <Pressable
          onPress={() => {
            if (
              flashcardPreviewDetailsData?.userProgress?.status ===
                'COMPLETED' &&
              flashcardPreviewDetailsData?.attestationDetail
  ...
  const AssignmentnModal = () => {
    return (
      <RNModal
        ...
        onDismiss={() => setAssModalVisible(false)}>
        <View
          style={{
          ...
          <RNImage
            onPress={() => setAssModalVisible(false)}
            source={IMAGES.VectorIcon}
            style={{width: 17, height: 17, left: scale(230), bottom: scale(15)}}
          />
          <RNText
            style={{paddingBottom: scale(5)}}
            TextAlignCenter
            large
            semiBold>
            Assignment Overview
          </RNText>
          <RNText textColor="#9398A4" style={{fontSize: scale(10)}}>
            Complete your assignment within the time limit
          </RNText>
          <View style={{flexDirection: 'row', marginTop: scale(30)}}>
            <RNText style={{fontSize: 11.5}}>Assessment Type :</RNText>
            <RNText style={{left: scale(7), fontSize: 11.5}} semiBold>
              Timed Assignment
            </RNText>
          </View>
          ...
          <View style={{flexDirection: 'row', marginTop: scale(5)}}>
            ...
            <RNText style={{left: scale(5), fontSize: 11.5}} semiBold>
              Single Attempt
            </RNText>
          </View>

          <RNText TextAlignCenter style={{marginTop: scale(40)}} large semiBold>
            This assignment has a 1h 20m Time limit.
          </RNText>
          <RNText
            style={{marginTop: scale(20), fontSize: 13}}
            textColor="#667085">
            Please ensure to submit your response within the allotted time to
            avoid penalties.
          </RNText>
          ...
          <View
            ...
            <RNImage
              source={IMAGES.FrameBulb}
              style={{width: 15, height: 15}}
            />
            <RNText style={{left: 5}} small>
              Quick Tips
            </RNText>
          </View>
          <View
            ...
            />
            <RNText style={{fontSize: 12, left: 5}} textColor="#5E5E5ECC">
  ...
  const ChapterItem: React.FC<ChapterItemProps> = ({
    ...
    return (
      <View style={styles.examMainView}>
        ...
        {showDetailsBoolean && chapter?.materials?.map(renderMaterialItem)}
        ...
        <Pressable
          ...
          <RNImage
            source={
              showDetailsBoolean ? IMAGES.chevronDown : IMAGES.chevronDown
            }
  ...
  const ScromChapterFunction = () => {
    return (
      <View style={styles.examMainView}>
        <RNText textColor={COLORS.TEXTCOLOR} large>
          Chapters
        </RNText>
        {scromChapterByData?.chapters &&
        scromChapterByData.chapters.length > 0 ? (
          scromChapterByData.chapters.map((item: any, index: any) => (
            <View
              key={index}
              style={[
                styles.secondMaterialView,
                {marginLeft: scale(5), marginTop: scale(7)},
              ]}>
              <RNImage
                source={IMAGES.fileLines}
