...
import { getGamificationMonthLeaderboardRequestAction } from '../module/action';
...
const MonthlyTabComponent = (props: any) => {
    const dispatch = useDispatch();
    const { gamificationMonthLeaderboardData, gamificationMonthLeaderboardLoading } = leaderboardSelector(); const [secondTimeBoardData, setSecondAllTimeBoardData] = useState<any>([]);
    ...
    const leaderboardMonth = () => {
        ...
        dispatch(getGamificationMonthLeaderboardRequestAction({ body, callback }))
    }

    useEffect(() => {
        if (gamificationMonthLeaderboardData?.items?.length) {
            if (gamificationMonthLeaderboardData?.items?.length > 5) {
                setFirstAllTimeBoardData(gamificationMonthLeaderboardData?.items?.slice(0, 3));
                setSecondAllTimeBoardData(gamificationMonthLeaderboardData?.items?.slice(3));
            } else {
                setFirstAllTimeBoardData([]);
                setSecondAllTimeBoardData(gamificationMonthLeaderboardData?.items);
            }
        }
            ...
            <GestureHandlerRootView style={{ flex: 1 }}>
                <View style={styles.actionsheet}>
                    <ScrollView showsVerticalScrollIndicator={false}>
                        ...
                        {gamificationMonthLeaderboardLoading ?
                                            ...
                                            >
                                                ...
                                                >
                                                    <RNText medium TextAlignCenter textColor={COLORS.TEXTCOLOR}>
                                                        {gamificationMonthLeaderboardData?.items?.length && gamificationMonthLeaderboardData?.items?.length > 5 ? index + 4 : index + 1}
                                                    </RNText>
                                                </View>
    ...
    return (
        <View style={styles.container}>
            {gamificationMonthLeaderboardData?.items?.length && gamificationMonthLeaderboardData?.items?.length > 5 ? FirstThreeView() : null}
            {RankBoardView()}
        </View>
    );
};
